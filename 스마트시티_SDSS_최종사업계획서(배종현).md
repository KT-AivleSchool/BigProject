...
*   **실제 행정 현황:** 서울시 내의 스마트 흡연부스(정화 필터 탑재형)를 도입하고 있는 단일 자치구(용산구, 노원구, 마포구 등 자치구역 1개)들은 적합한 장소를 찾지 못해 부지를 확보하는 데만 평균 6개월 이상의 행정 조율 시간이 낭비되고 있습니다.
...
*   **5단계: 배포 및 시나리오 QA (8주차)**
    *   *내용:* Docker Compose 배포 완료, 단일 자치구역(예: 용산구, 노원구, 마포구 등) 시나리오 시연 및 발표 자료 녹화.
    *   *담당:* 전원
...
*   **공무원 시나리오 시뮬레이션:** 선택한 자치구(예: 용산구 한강로동 등) 실데이터를 로딩하여 사용자가 우선순위(유동인구 가중치)를 변경했을 때, 실시간으로 지도 핀 위치가 이동하고 에이전트들의 찬반 논리가 유기적으로 피벗되어 PDF 보고서로 안정적으로 내려받아지는지 최종 사용성 평가 수행.
